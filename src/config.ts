...
export function resolveModel(flag: string | undefined, config: CopperheadConfig, env = process.env): ResolvedModel {
  ...
  throw new Error(
    'no model configured: pass --model codex, --model claude-code, or --model cursor (saved CLI login), --model lmstudio (a local LM Studio server), set COPPERHEAD_MODEL, set model in .copperhead/config.json, or provide OPENAI_API_KEY/ANTHROPIC_API_KEY',
  );
}
