...
- **Optional dependency**: `@anthropic-ai/claude-agent-sdk` is lazily `import()`ed inside `chat()`; users who never select `claude-code` neither install nor load it. It is a **documented manual install**, not declared in `package.json` — its hard peer on `@anthropic-ai/sdk >=0.93.0` conflicts with copperhead's pinned `^0.39.0`, and a peer conflict fails `npm install` even for an `optionalDependency` (see design D3).
...
- `package.json` — unchanged: `@anthropic-ai/claude-agent-sdk` is intentionally not declared (peer conflict with the pinned `@anthropic-ai/sdk`; see design D3); it is a documented manual install.
