...
### D3 — Optional dependency, lazily imported, but NOT declared in `package.json`

The provider `await import()`s `@anthropic-ai/claude-agent-sdk` (non-literal specifier, the `@maximem/synap-js-sdk` pattern) inside `chat()` wrapped in try/catch; a module-resolution failure becomes an actionable, non-retryable error ("install the optional dependency `@anthropic-ai/claude-agent-sdk`"). Users who never pass `--model claude-code` neither install nor load the SDK, and startup cost is unchanged. No `zod` is needed because no SDK tools are registered.

The SDK is **not** listed in `package.json` — not even under `optionalDependencies`. `@anthropic-ai/claude-agent-sdk@0.3.x` hard-**peers** on `@anthropic-ai/sdk >=0.93.0` (also `zod ^4`, `@modelcontextprotocol/sdk ^1.29`), which conflicts with copperhead's pinned `@anthropic-ai/sdk ^0.39.0`. `optionalDependencies` does not shield against a *peer* conflict — npm's ERESOLVE fails the whole install — so declaring it would break `npm install` for every user. Instead it is a documented manual install (`npm i @anthropic-ai/claude-agent-sdk`, possibly with `--legacy-peer-deps` until copperhead's core `@anthropic-ai/sdk` is bumped). *Rejected alternative:* bumping copperhead's `@anthropic-ai/sdk` to `>=0.93.0` to satisfy the peer and keep the optional-dep declaration — deferred as a larger, separately-verified change that would touch the existing Anthropic provider and pull in zod4 + the MCP SDK, out of scope for adding one provider.
