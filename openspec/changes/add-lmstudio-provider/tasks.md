...
- [x] 6.1 Update `openspec/specs/SPEC.md`: extend AC-3.10 to include `--model lmstudio`; add AC-3.13 (local model, no key, no cloud call, no fallback, actionable local failures); add `lmstudio.ts` to the §2 tree and §4.4 provider list; add `LMSTUDIO_BASE_URL` to the `.env.example` reference
