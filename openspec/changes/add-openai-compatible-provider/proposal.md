...
- New `compat` / `compat:<model-id>` route in `makeProvider()`, matching the prefix idiom already used by `codex`, `claude-code`, and `cursor`. `makeProvider` gains an optional settings argument; both production call sites already have config in scope.
