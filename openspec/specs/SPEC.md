...
- Rate-limit (429): exponential backoff ×3, then fail over to the other **keyed** provider if a key exists. Failover is between `openai` and `anthropic` only: a `codex`, `claude-code`, or `lmstudio` run exhausts its backoff and fails rather than switching to a billed provider (AC-3.11, AC-3.12)
