...
- Selection: `--model` flag > `COPPERHEAD_MODEL` env > config.json > an available key only when exactly one of `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` is set. Local and compatible endpoints are never auto-detected; they must be selected explicitly.
